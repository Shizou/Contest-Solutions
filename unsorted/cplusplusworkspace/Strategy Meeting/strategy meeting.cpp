...
set< pair< pair<LL,LL>,pair<LL,LL> > >paths;
...
void rec(int n,int d){
     if(n == N-1){
        ...
        for(int i = 0;i < (int)adj[n].size();++i){
            ...
            if(d >= 1  && d <= 4 )path.x.x |= (1 << (15*(d%5)+(d%5)+adj[n][i]));
            if(d >= 5  && d <= 8 )path.x.y |= (1 << (15*(d%5)+(d%5)+adj[n][i]));
            if(d >= 9  && d <= 12)path.y.x |= (1 << (15*(d%5)+(d%5)+adj[n][i]));
            if(d >= 13 && d <= 16)path.y.y |= (1 << (15*(d%5)+(d%5)+adj[n][i]));
